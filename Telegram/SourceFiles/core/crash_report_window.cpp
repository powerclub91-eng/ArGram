...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"ArGram"_q : title);
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"ArGram Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.telegramcrash"_q, u"ArGram crash report (*.telegramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
