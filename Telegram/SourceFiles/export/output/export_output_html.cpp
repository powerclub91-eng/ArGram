	...
	const auto serviceText = v::match(message.action.content, [&](
		...
		return !data.winners
			...
			? "Some winners of the giveaway were randomly selected by "
				"ArGram and received their prize."
			: (!data.credits && data.unclaimed)
			? "Some winners of the giveaway were randomly selected by "
				"ArGram and received private messages with giftcodes."
			: (data.credits && !data.unclaimed)
			? NumberToString(data.winners) + " of the giveaway was randomly "
				"selected by ArGram and received their prize."
			: NumberToString(data.winners) + " of the giveaway was randomly "
				"selected by ArGram and received private messages with "
				"giftcodes.";
	...
	if (data.credits > 0) {
		...
		result.append("<b>"
			...
			+ SerializeString((data.quantity > 1)
				? "ArGram Premium Subscriptions"
				: "ArGram Premium Subscription")
