...
constexpr auto kGalaxyRotationInterval = crl::time(3000); // ArGram: 3s
...
const BackgroundState &ChatTheme::backgroundState(QSize area) {
	...
	// ArGram: start Galaxy auto-rotation timer
	if (!_galaxyRotationTimer) {
		_galaxyRotationTimer.emplace([=] {
			rotateComplexGradientBackground();
			generateNextBackgroundRotation();
		});
		_galaxyRotationTimer->callEach(kGalaxyRotationInterval);
	}
